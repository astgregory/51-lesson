...
Улучшите функционал приложения, создав систему аутентификации, которая работает следующим образом:
...
Улучшение может быть реализовано через расширение `src/ui/components.py` для создания окна входа и добавление логики в `src/main.py` для обработки ключа и PIN. Хранение ключа и PIN можно реализовать через расширение функционала модуля кеширования `src/ui/components.py`. Окно входа должно содержать кнопку сброса ключа для удобства пользователя.


Таким образом, в существующее приложение необходимо интегрировать систему, которая будет требовать ввод секретного ключа `openRouter.ai` при первом входе, а затем позволит вход по пину (4 цифры).

## Вывод:
В ходе проделанной работы, была реализована система аутентификации: первый ввод api ключа с проверкой баланса → генерация и сохранение 4-значного PIN → последующие входы по PIN или сброс ключа. PIN всегда требуется при старте. `ChatCache.save_auth_data, get_auth_data, has_auth_data, clear_auth_data` — реализованы и используют SQLite, т.е. данные сохраняются.
...
2. **Клонирование проекта**
   ...
   git clone https://github.com/astgregory/51-lesson.git
